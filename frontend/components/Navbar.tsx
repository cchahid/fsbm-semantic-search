...
export function Navbar() {
  ...
  return (
    <header className="relative sticky top-0 z-50 w-full border-b border-slate-200/80 bg-white/90 py-5 shadow-sm backdrop-blur-md">
      <div className="mx-auto flex w-full max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex max-w-2xl flex-1 justify-center mx-8">
          {pathname === '/' && (
            <form onSubmit={handleSearch} className="relative hidden w-full min-w-0 md:block" role="search">
              <SearchIcon />
              <Input
                value={query}
                onChange={(event) => setQuery(event.target.value)}
                className="h-12 rounded-lg border-slate-300 bg-white/70 px-4 py-2.5 pl-10 pr-24 text-base shadow-sm focus-visible:border-blue-600 focus-visible:ring-blue-600/20"
                placeholder="Search papers, topics, or authors..."
                aria-label="Search research papers"
              />
              <Button type="submit" className="absolute right-1 top-1 h-10 bg-blue-700 px-4 text-base hover:bg-blue-900">
                Search
              </Button>
            </form>
          )}
        ...
        <nav className="flex-shrink-0 flex gap-8 items-center whitespace-nowrap text-base font-medium" aria-label="Primary navigation">
          {links.map((link) => {
